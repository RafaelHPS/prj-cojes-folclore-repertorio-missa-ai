...
export async function fetchTopSongs(teamId: string, limit = 5) {
  ...
  for (const row of (data ?? []) as unknown as { song_id: string; songs: { id: string; title: string; artist: string | null } | null }[]) {
