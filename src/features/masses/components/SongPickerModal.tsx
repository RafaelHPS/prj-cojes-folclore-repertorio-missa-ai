...
export function SongPickerModal({ part, songs, isAdding, onSelect, onClose }: Props) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-on-surface/40 sm:items-center px-4 pb-4 sm:pb-0"
      onClick={onClose}
    >
      <div
        role="dialog"
              ...
              {filtered.map((song) => (
                <li key={song.id}>
                  <button
                    ...
                    disabled={isAdding}
                    className="flex w-full items-center justify-between px-5 py-3.5 text-left transition hover:bg-surface-container-low disabled:opacity-60"
                  >
                    <div className="min-w-0">
                      <p className="font-semibold text-on-surface truncate">{song.title}</p>
                      {song.artist && (
                        <p className="mt-0.5 text-xs text-outline truncate">{song.artist}</p>
                      )}
                    </div>
                    {song.key && (
                      <span className="ml-3 flex-shrink-0 rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-bold text-primary">
                        {song.key}
                      </span>
                    )}
                  </button>
