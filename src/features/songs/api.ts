...
export async function createSong(teamId: string, form: SongForm): Promise<Song> {
...
export async function updateSong(id: string, form: SongForm): Promise<Song> {
  const { data, error } = await supabase
    .from('songs')
    .update({
      title: form.title.trim(),
...
export async function removeSongFile(
  ...
  // remove qualquer arquivo com esse prefixo (independente da extensão)
